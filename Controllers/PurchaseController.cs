        ...
        public ActionResult GetAllPurchases()
        {
            ...
            while (dt.Read())
            {
                listsub.Add(new clsPurchase()
                {
                    ...
                    RefrenceNumber = Convert.ToInt32(dt["RefrenceNumber"])
                });
            }
            ...
            if (id > 0)
            {
                country = (from c in db.tblPurchases
                           ...
                           select new clsPurchase
                           {
                               ...
                               RefrenceNumber = c.RefrenceNumber,
                               ItemName = db.tblPurchaseLines.Where(x => x.PurchaseId == c.PurchaseId).Select(x => x.ItemName).FirstOrDefault(),
                               Qty = db.tblPurchaseLines.Where(x => x.PurchaseId == c.PurchaseId).Select(x => x.Qty).FirstOrDefault(),
        ...
        public ActionResult AddUpdatePurchase(clsPurchase purchase, string childData)
        {
            ...
            try
            {
                ...
                if (PurchaseList.Count != 0)
                {
                    ...
                    {
                        dtPurchase.Rows.Add(new object[] { i + 1, PurchaseList[i].ItemName, PurchaseList[i].Qty, PurchaseList[i].Rate });
                    }
                }
                else
                {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("spInsertUpdatePurchase", con))
                    {
                        ...
                        cmd.Parameters.Add("@RefrenceNumber", SqlDbType.Int).Value = st.RefrenceNumber;
