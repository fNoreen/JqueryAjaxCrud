    ...
    public class clsSqlPurchase
    {
        ...
        public static DataTableReader getPurchaseList(string start, string length, string sorting)
        {
            ...
            using (SqlConnection con = new SqlConnection(connection))
            {
                ...
                cmd.CommandText = " select isnull(s.PurchaseId,0) as 'PurchaseId',isnull(s.PurchaseDate,'0000-00-00 00:00:00.000') as 'PurchaseDate', isnull(s.RefrenceNumber,0) as 'RefrenceNumber' from tblPurchase s " + sorting + " OFFSET " + voffset + " ROWS  FETCH NEXT " + length + " ROWS ONLY";
                cmd.Connection = con;
                SqlDataReader r = cmd.ExecuteReader();
                tdt.Load(r);
                r.Close();
